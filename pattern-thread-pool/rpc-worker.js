...
console.log(CORES)
const STRATEGY = new Set(['roundrobin', 'random', 'leastbusy']);
...
module.exports = class RpcWorkerPool {
  constructor(path, size = 0, strategy = 'roundrobin') {
    ...
    if(!STRATEGY.has(strategy)) throw new TypeError('invalid strategy');
    ...
    for (let i = 0; i < this.size; i++) {
      ...
      this.workers.push({ worker, in_fligth_commands: new Map( )});
  ...
  onMessageHandler(msg, worker_id) {
    ...
    const {resolve, reject} = worker.in_fligth_commands.get(id);
    worker.in_fligth_commands.delete(id);
  ...
  exec(method, ...args) {
    ...
    worker.in_fligth_commands.set(id, {resolve, reject});
  ...
  getWorker() {
    ...
    if (this.strategy === 'random') {
      ...
      for (let i = 0; i < this.size; i++) {
        ...
        if (worker.in_fligth_commands.size < min) {
          min = worker.in_fligth_commands.size;
          id = i
        }
      }
    }
